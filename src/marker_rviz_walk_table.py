...
def addSupportFoot(x,y,q_z,q_w,name,i):
	...
	mesh = declareMarker(name, ind, "package://estimation/mesh/RobotFoot.stl")
...
class marker_pub:
	...
	def callback(self, traj,est,nmpc):
		...
		if len(x_1) == 0 or len(x_2) == 0:
			...
			if len(x_1) == 0:
				pos1_wrt_table = setFrame(x_est[ind], y_est[ind], theta_est[ind],self.dist_to_table,1)
				frame_msg.transforms.append(pos1_wrt_table)
				marker_est1 = addCurrentEstRobot(x_est,y_est,theta_est[ind],self.dist_to_table,self.N_0,1)
				self.pub.publish(marker_est1)

			if len(x_2) == 0:
				pos2_wrt_table = setFrame(x_est[ind], y_est[ind], theta_est[ind],self.dist_to_table,2)
				frame_msg.transforms.append(pos2_wrt_table)
				marker_est2 = addCurrentEstRobot(x_est,y_est,theta_est[ind],self.dist_to_table,self.N_0,2)
