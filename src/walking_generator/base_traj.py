...
class BaseGeneratorTraj(object):
    ...
    def _update_selection_matrices(self):
        ...
        if (self.v_kp1 == 0).all():
            ...
            if (numpy.sum(numpy.diff(self.C_kp1_x_ref[:8])) != 0.0).any() \
            or (numpy.sum(numpy.diff(self.C_kp1_y_ref[:8])) != 0.0).any() \
            or (numpy.sum(numpy.diff(self.C_kp1_q_ref[:8])) != 0.0).any():
    ...
    def update(self):
        ...
        # print("traj ref : ",self.traj_ref[0][0],self.traj_ref[0][7],self.traj_ref[0][-1])
